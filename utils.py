...
class solver_utils:
    ...
    def plot_field_with_cut(self, Xv, E, Qc, field="rho", y0=0.0, levels=10,
                            show_mesh=True,plot_exact=True, cmap="viridis"):
        """
        ...
        if plot_exact:
            qline_exact=self.vortex.init_Q(np.vstack((xline,yline)).T,
                         x0=10.0+self.uinf*self.time,y0=5,uinf=0.5,vinf=0)
    ...
    def lsq_wts_batched2(self,dx, mask, r=-1, eps=1e-12):
        """
    ...
    def lsq_wts_batched(self, dx, mask, r=0, eps=1e-14, xp=np):
        """
        Batched LSQ weights for gradient reconstruction in 2D/3D.
        
        Parameters
        ----------
        dx : (N, max_neighbors, dim) array (numpy or cupy)
        Relative coordinates (x_j - x0_i).
        mask : (N, max_neighbors) bool array
        True where a neighbor is valid, False for padding.
        r : float
        ...
        Returns
        -------
        wgts : (N, max_neighbors, dim) array
        LSQ weights corresponding to each neighbor and dimension.
        """
        N, max_neighbors, dim = dx.shape

        # Distance + weights
        dist = xp.linalg.norm(dx, axis=2)       # (N, max_neighbors)
        w = xp.where(mask, dist**r, 0.0)        # zero-out padding

        # Build normal matrices A = X^T W X (N,dim,dim)
        A = np.einsum('bki,bk,bkj->bij', dx, w, dx)
        # Invert A (batched)
        if dim == 2:
            det = A[:,0,0]*A[:,1,1] - A[:,0,1]*A[:,1,0]
            det = xp.where(abs(det) < eps, eps, det)
            Ainv = xp.zeros_like(A)
            Ainv[:,0,0] =  A[:,1,1] / det
            Ainv[:,1,1] =  A[:,0,0] / det
            Ainv[:,0,1] = -A[:,0,1] / det
            Ainv[:,1,0] = -A[:,1,0] / det
            ...
            det = xp.linalg.det(A)
            det = xp.where(abs(det) < eps, eps, det)
            Ainv = xp.linalg.inv(A)  # batched in numpy/cupy
        else:
            raise ValueError("dim must be 2 or 3")

        # Weighted coordinates (N,max_neighbors,dim)
        WX = w[:,:,None] * dx

        # Apply inverse: (N,max_neighbors,dim) @ (N,dim,dim)
        wgts = WX @ Ainv
    ...
    def reconstruct_faces(self,Uc, e2f, e2n, grad_weights, stencil_dx,
                          face_coords, centroid_coords, xp=np):
        """
        Vectorized reconstruction of left/right states at faces.
    
        Parameters
        ----------
        Uc : (Ne, nv) array
            Element-centered values (Ne elements, nv variables)
        e2f : (Nf, 2) array
            Edge-to-element connectivity (left/right element per face)
        e2n : (Ne, max_stencil) array
            Neighbor element indices per element for gradient reconstruction
        grad_weights : (Ne, max_stencil, dim) array
            Gradient weights for each stencil neighbor
        stencil_dx : (Ne, max_stencil, dim) array
            Vector from element centroid to neighbor centroids
        face_coords : (Nf, dim) array
            Coordinates of face centers
        centroid_coords : (Ne, dim) array
            Element centroids
        xp : module
            np or cp (for GPU)
        ...
        -------
        UL, UR : (Nf, nv) arrays
            Left and right reconstructed states at faces
        """
    
        Ne, nv = Uc.shape
        Nf, dim = face_coords.shape
        max_stencil = grad_weights.shape[1]
    
        # --- Step 1: Compute gradients at each element ---
        # gradU: (Ne, nv, dim)
        gradU = xp.zeros((Ne, nv, dim), dtype=Uc.dtype)
    
        # gather neighbor differences
        neighbors = e2n  # (Ne, max_stencil)
        mask = neighbors >= 0  # boolean mask of valid neighbors
    
        # deltaU = U_neighbor - U_center
        # We'll broadcast Uc over stencil axis
        U_neighbors = xp.where(mask[..., None], Uc[neighbors.clip(0)], 0)  # shape (Ne, max_stencil, nv)
        U_center    = xp.repeat(Uc[:, None, :], max_stencil, axis=1)       # (Ne, max_stencil, nv)
        deltaU      = U_neighbors - U_center                                 # (Ne, max_stencil, nv)
        
        # multiply by grad_weights (Ne, max_stencil, dim) -> (Ne, max_stencil, nv, dim)
        contrib = deltaU[..., None] * grad_weights[:, :, None, :]  # broadcast: (Ne, max_stencil, nv, dim)
        
        # sum over stencil neighbors
        gradU = xp.sum(contrib, axis=1)  # shape (Ne, nv, dim)
    
        # --- Step 2: Reconstruct left/right face states ---
        eL = e2f[:, 0]
        eR = e2f[:, 1]
    
        # vector from element centroid to face center: (Nf, dim)
        dxL = face_coords - centroid_coords[eL]
        dxR = face_coords - centroid_coords[eR]

        # UL = Uc[eL] + gradU[eL] . dxL
        UL = Uc[eL] + xp.einsum('evi,fi->ev', gradU[eL], dxL)
        UR = Uc[eR] + xp.einsum('evi,fi->ev', gradU[eR], dxR)
